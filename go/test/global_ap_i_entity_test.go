...
func TestGlobalApIEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GlobalApI(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GlobalApI(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		globalApIRef01Data = core.ToMapAny(entityData(globalApIRef01DataResult))
		...
		_, globalApIRef01ListOk := globalApIRef01ListResult.([]any)
...
func global_ap_iBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DUTCH_CUSTOMER_DATA_TEST_GLOBAL_AP_I_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DUTCH_CUSTOMER_DATA_TEST_GLOBAL_AP_I_ENTID"])
