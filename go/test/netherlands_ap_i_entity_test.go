...
func TestNetherlandsApIEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.NetherlandsApI(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.NetherlandsApI(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DUTCH_CUSTOMER_DATA_TEST_NETHERLANDS_AP_I_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func netherlands_ap_iBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DUTCH_CUSTOMER_DATA_TEST_NETHERLANDS_AP_I_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DUTCH_CUSTOMER_DATA_TEST_NETHERLANDS_AP_I_ENTID"])
