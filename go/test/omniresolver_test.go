...
import (
	...
	omni "github.com/voxgig-sdk/dutch-customer-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DutchCustomerDataError); is {
				out := map[string]any{
					"name":    "DutchCustomerDataError",
...
func omniCtx(arg any, client *sdk.DutchCustomerDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DutchCustomerDataSDK); is {
			client = live
