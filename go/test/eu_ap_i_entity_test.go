...
func TestEuApIEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.EuApI(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.EuApI(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		euApIRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.eu_ap_i")))
		...
		euApIRef01DataDt0LoadResult := core.ToMapAny(entityData(euApIRef01DataDt0Loaded))
...
func eu_ap_iBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DUTCH_CUSTOMER_DATA_TEST_EU_AP_I_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DUTCH_CUSTOMER_DATA_TEST_EU_AP_I_ENTID"])
