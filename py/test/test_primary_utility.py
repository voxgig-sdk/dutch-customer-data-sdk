...
_runner = makeRunner(TEST_JSON_FILE, DutchCustomerDataSDK.test(None, None))
...
class TestPrimaryUtility:
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = DutchCustomerDataSDK.test(None, setup.get('a') or {})
    ...
    def test_operator_basic(self):
        def subject(vin):
            if not isinstance(vin, dict):
                vin = {}
            op = DutchCustomerDataOperation(vin)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = DutchCustomerDataSDK.test(None, setup.get('a') or {})
