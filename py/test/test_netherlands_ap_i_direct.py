...
class TestNetherlandsApIDirect:

    def test_should_direct_list_netherlands_ap_i(self):
        setup = _netherlands_ap_i_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-netherlands_ap_i", "live" if setup["live"] else "unit")
        if _skip:
...
def _netherlands_ap_i_direct_setup(mockres):
