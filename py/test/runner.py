# DutchCustomerData SDK test runner
...
class DutchCustomerDataTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DutchCustomerDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DutchCustomerDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DutchCustomerDataTestRunner.getenv("DUTCH_CUSTOMER_DATA_TEST_LIVE")
        override = DutchCustomerDataTestRunner.getenv("DUTCH_CUSTOMER_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DutchCustomerDataTestRunner.getenv(key)
        ...
        explain = DutchCustomerDataTestRunner.getenv("DUTCH_CUSTOMER_DATA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DUTCH_CUSTOMER_DATA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DutchCustomerDataTestRunner._test_control is not None:
            return DutchCustomerDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DutchCustomerDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DutchCustomerDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DutchCustomerDataTestRunner.load_test_control()
...
def load_env_local():
    DutchCustomerDataTestRunner.load_env_local()


def env_override(m):
    return DutchCustomerDataTestRunner.env_override(m)


def entity_data(v):
    return DutchCustomerDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DutchCustomerDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DutchCustomerDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DutchCustomerDataTestRunner.load_test_control()


def live_client_options():
    return DutchCustomerDataTestRunner.live_client_options()


def live_delay_ms():
    return DutchCustomerDataTestRunner.live_delay_ms()
