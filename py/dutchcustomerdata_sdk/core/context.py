...
from dutchcustomerdata_sdk.core.helpers import get_ctx_prop, to_map
...
class DutchCustomerDataContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = DutchCustomerDataControl()
        ...
        if isinstance(sp, DutchCustomerDataSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, DutchCustomerDataResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, DutchCustomerDataResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return DutchCustomerDataOperation({})
    ...
    def make_error(self, code, msg):
        return DutchCustomerDataError(code, msg, self)
