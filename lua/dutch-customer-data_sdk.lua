...
require("dutch-customer-data_types")
...
local DutchCustomerDataSDK = {}
DutchCustomerDataSDK.__index = DutchCustomerDataSDK
...
DutchCustomerDataSDK._make_feature = _make_feature


function DutchCustomerDataSDK.new(options)
  local self = setmetatable({}, DutchCustomerDataSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function DutchCustomerDataSDK:options_map()
...
function DutchCustomerDataSDK:get_utility()
  return Utility.copy(self._utility)
end


function DutchCustomerDataSDK:get_root_ctx()
  return self._rootctx
end


function DutchCustomerDataSDK:prepare(fetchargs)
...
function DutchCustomerDataSDK:direct(fetchargs)
...
function DutchCustomerDataSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function DutchCustomerDataSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "DutchCustomerDataSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function DutchCustomerDataSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DutchCustomerDataSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DutchCustomerDataSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function DutchCustomerDataSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = DutchCustomerDataSDK.new(sdkopts)
  sdk.mode = "test"
