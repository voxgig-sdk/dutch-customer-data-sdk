...
local GlobalApIEntity = {}
GlobalApIEntity.__index = GlobalApIEntity


function GlobalApIEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GlobalApIEntity)
  self._name = "global_ap_i"
  self._client = client
...
function GlobalApIEntity:get_name()
  return self._name
end


function GlobalApIEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GlobalApIEntity.new(self._client, opts)
end
...
function GlobalApIEntity:mark_deleted()
  self._deleted = true
end


function GlobalApIEntity:deleted()
  return true == self._deleted
end


function GlobalApIEntity:data_set(args)
  if args ~= nil then
...
function GlobalApIEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GlobalApIEntity:match_set(args)
  if args ~= nil then
...
function GlobalApIEntity:match_get()
...
function GlobalApIEntity:stream(action, args, callopts)
...
function GlobalApIEntity:_run_op(ctx, post_done)
  local utility = self._utility
