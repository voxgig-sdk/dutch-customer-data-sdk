...
local EuApIEntity = {}
EuApIEntity.__index = EuApIEntity


function EuApIEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EuApIEntity)
  self._name = "eu_ap_i"
  self._client = client
...
function EuApIEntity:get_name()
  return self._name
end


function EuApIEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EuApIEntity.new(self._client, opts)
end
...
function EuApIEntity:mark_deleted()
  self._deleted = true
end


function EuApIEntity:deleted()
  return true == self._deleted
end


function EuApIEntity:data_set(args)
  if args ~= nil then
...
function EuApIEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EuApIEntity:match_set(args)
  if args ~= nil then
...
function EuApIEntity:match_get()
...
function EuApIEntity:stream(action, args, callopts)
...
function EuApIEntity:_run_op(ctx, post_done)
  local utility = self._utility
