...
describe("GlobalApIDirect", function()
  it("should direct-list-global_ap_i", function()
    local setup = global_ap_i_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-global_ap_i", setup.live and "live" or "unit")
  ...
  it("should direct-load-global_ap_i", function()
    local setup = global_ap_i_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-global_ap_i", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function global_ap_i_direct_setup(mockres)
