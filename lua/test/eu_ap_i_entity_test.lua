...
describe("EuApIEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:EuApI(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:EuApI(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:EuApI(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = eu_ap_i_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "eu_ap_i." .. _op, _live and "live" or "unit")
    ...
    local eu_ap_i_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.eu_ap_i")))
    local eu_ap_i_ref01_data = nil
    if #eu_ap_i_ref01_data_raw > 0 then
      eu_ap_i_ref01_data = helpers.to_map(eu_ap_i_ref01_data_raw[1][2])
    end

    -- LIST
    local eu_ap_i_ref01_ent = client:EuApI(nil)
    local eu_ap_i_ref01_match = {}

    local eu_ap_i_ref01_list_result, err = eu_ap_i_ref01_ent:list(eu_ap_i_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(eu_ap_i_ref01_list_result)

    -- LOAD
    local eu_ap_i_ref01_match_dt0 = {
...
function eu_ap_i_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/eu_ap_i/EuApITestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read eu_ap_i test data: " .. entity_data_file)
  end
