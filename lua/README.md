# DutchCustomerData Lua SDK

The Lua SDK for the DutchCustomerData API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:EuApI(nil):list(nil, nil)
...
```lua
local result, err = client:EuApI(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:DutchCustomerData(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── dutch-customer-data_sdk.lua    -- Main SDK module
...
The main module (`dutch-customer-data_sdk`) exports the SDK constructor
