# dutch-customer-data-mcp

[MCP](https://modelcontextprotocol.io) server exposing the DutchCustomerData SDK as
two agent tools — `dutch-customer-data_list` and `dutch-customer-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dutch-customer-data-mcp)
make build

# 2. Provide credentials via the environment
export DUTCH_CUSTOMER_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user dutch-customer-data \
  -- /absolute/path/to/dutch-customer-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./dutch-customer-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// dutch-customer-data_list: first page of records
{ "entity": "eu_ap_i" }
{ "entity": "eu_ap_i", "query": { } }

// dutch-customer-data_load: one record by id
{ "entity": "eu_ap_i", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dutch-customer-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dutch-customer-data \
     -- "$PWD"/dist/*/dutch-customer-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dutch-customer-data_list` and `dutch-customer-data_load` tools now appear
   in new sessions. Ask the agent to *"list eu_ap_i using dutch-customer-data"*
   and it calls `dutch-customer-data_list` with `{"entity":"eu_ap_i"}`.
...
```sh
export DUTCH_CUSTOMER_DATA_APIKEY=sk_live_xxx            # API key
export DUTCH_CUSTOMER_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `dutch-customer-data_list` tool
...
### Call the `dutch-customer-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `dutch-customer-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dutch-customer-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DUTCH_CUSTOMER_DATA_APIKEY` | API key sent with every request. |
| `DUTCH_CUSTOMER_DATA_BASE` | Optional override of the API base URL. |
...
```sh
./dutch-customer-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dutch-customer-data_load","arguments":{"entity":"eu_ap_i","query":{"id":1}}}}'
```
