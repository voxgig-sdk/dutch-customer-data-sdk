...
boru-driven command-line client **and** interactive REPL for the DutchCustomerData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dutch-customer-data-cli)
make build

# 2. See usage (words, entities, env vars)
./dutch-customer-data-cli --help
...
./dutch-customer-data-cli load 1 eu_ap_i            # {id:1} shorthand
./dutch-customer-data-cli load '{id:1}' eu_ap_i       # explicit match map
./dutch-customer-data-cli list global_ap_i

# 5. Override the API base URL for a single call
DUTCH_CUSTOMER_DATA_BASE=https://api.example.com ./dutch-customer-data-cli list eu_ap_i

# 6. No arguments -> interactive REPL
./dutch-customer-data-cli
dutch-customer-data> list eu_ap_i
dutch-customer-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dutch-customer-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/dutch-customer-data-cli list eu_ap_i
   ```
...
```sh
./dutch-customer-data-cli load 1 eu_ap_i          # scalar shorthand for {id:1}
./dutch-customer-data-cli load '{id:1}' eu_ap_i     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `dutch-customer-data>`). Each line is
evaluated as its own boru expression:

```text
$ ./dutch-customer-data-cli
dutch-customer-data> list eu_ap_i
dutch-customer-data> /help
dutch-customer-data> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `eu_ap_i`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dutch-customer-data-cli`. |
...
(`./dutch-customer-data-cli <expr>`) and interactively (the REPL), and expressions compose the
