// Command DutchCustomerData-cli is a CLI + REPL for the DutchCustomerData SDK,
...
const prompt = "dutch-customer-data"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "eu_ap_i global_ap_i netherlands_ap_i"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DUTCH_CUSTOMER_DATA_APIKEY (api key), DUTCH_CUSTOMER_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DUTCH_CUSTOMER_DATA_APIKEY carries the API key and
	// DUTCH_CUSTOMER_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DUTCH_CUSTOMER_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DUTCH_CUSTOMER_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDutchCustomerDataSDK(opts)
