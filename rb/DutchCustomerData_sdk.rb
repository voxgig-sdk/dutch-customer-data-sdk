...
class DutchCustomerDataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DutchCustomerDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DutchCustomerDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DutchCustomerDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    DutchCustomerDataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = DutchCustomerDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DutchCustomerDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => DutchCustomerDataError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = DutchCustomerDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DutchCustomerDataError.new(
        "graphql_error", "DutchCustomerDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = DutchCustomerDataSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
