...
module DutchCustomerDataOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if DutchCustomerDataOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = DutchCustomerDataOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| DutchCustomerDataOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || DutchCustomerDataOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = DutchCustomerDataSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = DutchCustomerDataError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { DutchCustomerDataOmni.errify(err) }
