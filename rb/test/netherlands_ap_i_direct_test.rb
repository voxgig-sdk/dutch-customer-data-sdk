...
class NetherlandsApIDirectTest < Minitest::Test
  def test_direct_list_netherlands_ap_i
    setup = netherlands_ap_i_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-netherlands_ap_i", setup[:live] ? "live" : "unit")
    if _should_skip
...
def netherlands_ap_i_direct_setup(mockres)
