...
class EuApIEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.EuApI(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.EuApI(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DUTCH_CUSTOMER_DATA_TEST_EU_AP_I_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    eu_ap_i_ref01_data_dt0_load_result = Helpers.to_map(eu_ap_i_ref01_data_dt0_loaded.respond_to?(:data_get) ? eu_ap_i_ref01_data_dt0_loaded.data_get : eu_ap_i_ref01_data_dt0_loaded)
...
def eu_ap_i_basic_setup(extra)
  ...
  entid_env_raw = ENV["DUTCH_CUSTOMER_DATA_TEST_EU_AP_I_ENTID"]
