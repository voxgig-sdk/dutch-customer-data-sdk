...
The Ruby SDK for the DutchCustomerData API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.EuApI(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.DutchCustomerData(nil).load(
  { "id" => "test01" }, nil
)
