...
//     (\`new DutchCustomerDataSDK(...)\` / \`DutchCustomerDataSDK.test(...)\` become
//     \`DutchCustomerDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'DutchCustomerDataSDK'
...
const TEST_SEED = {"entity":{"eu_ap_i":{"test01":{"id":"test01"}},"global_ap_i":{"test01":{"id":"test01"}},"netherlands_ap_i":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DutchCustomerDataSDK(...)\` and any
// \`DutchCustomerDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DutchCustomerDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DutchCustomerDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
