...
describe('NetherlandsApIDirect', async () => {
  ...
  test('direct-list-netherlands_ap_i', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-netherlands_ap_i', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DUTCH_CUSTOMER_DATA_TEST_NETHERLANDS_AP_I_ENTID']
