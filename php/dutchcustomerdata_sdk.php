...
class DutchCustomerDataSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = DutchCustomerDataHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = DutchCustomerDataHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!DutchCustomerDataFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, DutchCustomerDataFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return DutchCustomerDataUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = DutchCustomerDataHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = DutchCustomerDataHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = DutchCustomerDataHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new DutchCustomerDataError($op . "_allow",
                "DutchCustomerDataSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = DutchCustomerDataHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new DutchCustomerDataError("graphql_error",
                "DutchCustomerDataSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new DutchCustomerDataSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
