...
// DutchCustomerData SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = DutchCustomerDataOmni::makeStructRunner(
    ...
    {
        return DutchCustomerDataOmni::objmodel(DutchCustomerDataOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return DutchCustomerDataOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return DutchCustomerDataOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(DutchCustomerDataOmni::structfix($now, Struct::undef()))
                        !== json_encode(DutchCustomerDataOmni::structfix($val, Struct::undef()))) {
