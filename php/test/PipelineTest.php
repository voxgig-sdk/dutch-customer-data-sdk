...
// DutchCustomerData SDK pipeline test
//
...
{
    private static function utility(): DutchCustomerDataUtility
    {
        return new DutchCustomerDataUtility();
    }
    ...
    private static function ctx(array $over = []): DutchCustomerDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DutchCustomerDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DutchCustomerDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DutchCustomerDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DutchCustomerDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DutchCustomerDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DutchCustomerDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DutchCustomerDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DutchCustomerDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DutchCustomerDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DutchCustomerDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DutchCustomerDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DutchCustomerDataResponse([]),
            'result' => new DutchCustomerDataResult([]),
        ]);
        [, $err] = DutchCustomerDataMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new DutchCustomerDataSpec([]),
            'response' => null,
            'result' => new DutchCustomerDataResult([]),
        ]);
        [, $err] = DutchCustomerDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DutchCustomerDataSpec(['step' => 's']),
            'response' => new DutchCustomerDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DutchCustomerDataResult([]),
        ]);
        [, $err] = DutchCustomerDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DutchCustomerDataSpec(['step' => 's']),
            'response' => new DutchCustomerDataResponse(self::resp(200, ['v' => 1])),
            'result' => new DutchCustomerDataResult([]),
        ]);
        [, $err] = DutchCustomerDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DutchCustomerDataSpec(['step' => 's']),
            'response' => new DutchCustomerDataResponse(self::resp(200, ['v' => 2])),
            'result' => new DutchCustomerDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DutchCustomerDataMakeResponse::call($ctx);
    ...
    {
        $preset = new DutchCustomerDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DutchCustomerDataSpec([]),
            'response' => new DutchCustomerDataResponse([]),
            'result' => new DutchCustomerDataResult([]),
        ...
        [$got, $err] = DutchCustomerDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DutchCustomerDataResult([])]);
        [, $err] = DutchCustomerDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DutchCustomerDataSpec([]), 'result' => null]);
        [, $err] = DutchCustomerDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DutchCustomerDataSpec(['step' => 's']),
            'result' => new DutchCustomerDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DutchCustomerDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DutchCustomerDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DutchCustomerDataSpec(['step' => 's']),
            'result' => new DutchCustomerDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DutchCustomerDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DutchCustomerDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DutchCustomerDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DutchCustomerDataSpec([]),
            'result' => new DutchCustomerDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DutchCustomerDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DutchCustomerDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DutchCustomerDataError('boom', 'boom');
        $utility->fetcher = function (DutchCustomerDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DutchCustomerDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DutchCustomerDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DutchCustomerDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DutchCustomerDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DutchCustomerDataResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        DutchCustomerDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DutchCustomerDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DutchCustomerDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DutchCustomerDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DutchCustomerDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DutchCustomerDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DutchCustomerDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DutchCustomerDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DutchCustomerDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DutchCustomerDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DutchCustomerDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DutchCustomerDataResult(['ok' => false])]);
        $this->expectException(DutchCustomerDataError::class);
        DutchCustomerDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DutchCustomerDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DutchCustomerDataMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        DutchCustomerDataMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DutchCustomerDataBaseFeature();
        $b = new DutchCustomerDataBaseFeature();
        DutchCustomerDataFeatureAdd::call($ctx, $a);
        DutchCustomerDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DutchCustomerDataBaseFeature
    {
        $f = new DutchCustomerDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DutchCustomerDataFeatureAdd::call($ctx, self::named_feature('a'));
        DutchCustomerDataFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        DutchCustomerDataFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        DutchCustomerDataFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        DutchCustomerDataFeatureAdd::call($ctx, $replace);
        ...
        DutchCustomerDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DutchCustomerDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DutchCustomerDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DutchCustomerDataPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        DutchCustomerDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DutchCustomerDataResponse(['headers' => null]),
            'result' => new DutchCustomerDataResult([]),
        ]);
        DutchCustomerDataResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        DutchCustomerDataResultBody::call($ctx);
