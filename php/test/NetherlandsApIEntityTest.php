...
class NetherlandsApIEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = DutchCustomerDataSDK::test(null, null);
        $ent = $testsdk->NetherlandsApI(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->NetherlandsApI(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->NetherlandsApI(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = netherlands_ap_i_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "netherlands_ap_i." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DUTCH_CUSTOMER_DATA_TEST_NETHERLANDS_AP_I_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $netherlands_ap_i_ref01_data = null;
        if (count($netherlands_ap_i_ref01_data_raw) > 0) {
...
function netherlands_ap_i_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/netherlands_ap_i/NetherlandsApITestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["netherlands_ap_i01", "netherlands_ap_i02", "netherlands_ap_i03"] as $k) {
    ...
    $entid_env_raw = getenv("DUTCH_CUSTOMER_DATA_TEST_NETHERLANDS_AP_I_ENTID");
