...
class EuApIEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = DutchCustomerDataSDK::test(null, null);
        $ent = $testsdk->EuApI(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->EuApI(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->EuApI(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = eu_ap_i_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "eu_ap_i." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DUTCH_CUSTOMER_DATA_TEST_EU_AP_I_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $eu_ap_i_ref01_data = null;
        if (count($eu_ap_i_ref01_data_raw) > 0) {
        ...
        $eu_ap_i_ref01_data_dt0_load_result = Helpers::to_map(is_object($eu_ap_i_ref01_data_dt0_loaded) && method_exists($eu_ap_i_ref01_data_dt0_loaded, 'data_get') ? $eu_ap_i_ref01_data_dt0_loaded->data_get() : $eu_ap_i_ref01_data_dt0_loaded);
...
function eu_ap_i_basic_setup($extra)
{
    Runner::load_env_local();

    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/eu_ap_i/EuApITestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["eu_ap_i01", "eu_ap_i02", "eu_ap_i03"] as $k) {
    ...
    $entid_env_raw = getenv("DUTCH_CUSTOMER_DATA_TEST_EU_AP_I_ENTID");
