...
class DutchCustomerDataMakeError
{
    public static function call(?DutchCustomerDataContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DutchCustomerDataContext([], null);
        }
        $op = $ctx->op ?? new DutchCustomerDataOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DutchCustomerDataResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DutchCustomerDataError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new DutchCustomerDataError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof DutchCustomerDataError) {
