...
// DutchCustomerData SDK retry feature
...
class DutchCustomerDataRetryFeature extends DutchCustomerDataBaseFeature
{
    ...
    public function init(DutchCustomerDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (DutchCustomerDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(DutchCustomerDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(DutchCustomerDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
