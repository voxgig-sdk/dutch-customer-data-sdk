...
The PHP SDK for the DutchCustomerData API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->EuApI(null)->list(null, null);
...
```php
[$result, $err] = $client->EuApI(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->DutchCustomerData(null)->load(
    ["id" => "test01"], null
);
