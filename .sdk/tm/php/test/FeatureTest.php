...
// DutchCustomerData SDK feature test
//
...
class FtCtrl extends DutchCustomerDataControl
{
...
class FtClient
{
    ...
    private DutchCustomerDataUtility $utility;

    public function __construct(DutchCustomerDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): DutchCustomerDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DutchCustomerDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DutchCustomerDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DutchCustomerDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DutchCustomerDataConfig::make_feature($name);
    ...
    private static function buildUrl(DutchCustomerDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DutchCustomerDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DutchCustomerDataResponse($fetched);
        }
    ...
    private function fail(DutchCustomerDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DutchCustomerDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
