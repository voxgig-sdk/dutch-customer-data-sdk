...
// DutchCustomerData SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class DutchCustomerDataMakePoint
{
    public static function call(DutchCustomerDataContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof DutchCustomerDataError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = DutchCustomerDataHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = DutchCustomerDataHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
