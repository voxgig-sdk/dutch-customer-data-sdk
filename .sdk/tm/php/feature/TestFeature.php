...
// DutchCustomerData SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class DutchCustomerDataTestFeature extends DutchCustomerDataBaseFeature
{
    ...
    public function init(DutchCustomerDataContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (DutchCustomerDataContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = DutchCustomerDataParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (DutchCustomerDataContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(DutchCustomerDataContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = DutchCustomerDataParam::call($ctx, $k);
