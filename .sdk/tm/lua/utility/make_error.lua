...
local function make_error_util(ctx, err)
  ...
  local msg = "DutchCustomerDataSDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = DutchCustomerDataError.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == DutchCustomerDataError then
