# DutchCustomerData SDK utility: make_request
...
module DutchCustomerDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DutchCustomerDataResponse.new({})
    ...
    if fetch_err
      ...
      response = DutchCustomerDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DutchCustomerDataResponse.new(fetched)
    else
